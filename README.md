...
TODO: Use approx equal in case checking.
...
Conceptually, if the variables in the `variable` field are equal to the numbers defined in the `cases` field (or within a range `!R`) then the corresponding value in the `then` field will be selected. If none of the cases match the value of the `default` field is taken. If your cases overlap by accident, the first matching case will be taken.
